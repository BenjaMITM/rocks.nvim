...
local function get_luarocks_lua_dir_from_luarocks()
    ...
    local ok, so = pcall(vim.system, { config.luarocks_binary, "--lua-version=5.4", "which", "luarocks.loader" }, nil)
    if not ok then
    ...
    local sc = so:wait()
    local result = sc.stdout and sc.stdout:match(vim.fs.joinpath("(%S+)", "5.4", "luarocks", "loader.lua"))
    return result
end
...
function loader.enable()
    ...
    if luarocks_lua_dir then
        package.path = package.path
            .. ";"
            .. table.concat({
                vim.fs.joinpath(luarocks_lua_dir, "5.4", "?.lua"),
                vim.fs.joinpath(luarocks_lua_dir, "5.4", "init.lua"),
            }, ";")
